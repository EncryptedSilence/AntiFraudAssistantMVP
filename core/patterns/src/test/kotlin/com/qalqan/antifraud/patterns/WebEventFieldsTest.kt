...
class WebEventFieldsTest {
    ...
    @Test fun `domainDisplayLocal`() {
        WebEventFields.lookup(web, "domainDisplayLocal") shouldBe "halykbank.kz"
    }

    @Test fun `isNewDomain`() {
        WebEventFields.lookup(web, "isNewDomain") shouldBe true
    }

    @Test fun `domainStatus as String`() {
        WebEventFields.lookup(web, "domainStatus") shouldBe "NEW"
    }

    @Test fun `webRiskScore`() {
        WebEventFields.lookup(web, "webRiskScore") shouldBe 35
    }

    @Test fun `unknown field returns null`() {
        WebEventFields.lookup(web, "ghost") shouldBe null
    }
}

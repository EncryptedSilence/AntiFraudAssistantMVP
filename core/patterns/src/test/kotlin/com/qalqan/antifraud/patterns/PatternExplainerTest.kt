...
class PatternExplainerTest {
    ...
    private fun pattern(
        ...
        conditions: List<PatternCondition> = listOf(
    ...
    fun `explanation level is the max across triggered patterns`() {
        val matches = listOf(
            pattern("p1", warning = warningHigh) to MatchResult(PatternId("p1"), true, 30, listOf(EventId("e1"))),
            pattern("p2", warning = warningCritical) to MatchResult(PatternId("p2"), true, 40, listOf(EventId("e2")))
        )
    ...
    fun `explainer emits at least 3 reasons when at least 3 conditions triggered`() {
        ...
        val matches = listOf(
            patternWith3Conds to MatchResult(

...
class SmsEventFieldsTest {
    ...
    @Test fun `containsCode`() {
        SmsEventFields.lookup(sms, "containsCode") shouldBe true
    }

    @Test fun `containsLink`() {
        SmsEventFields.lookup(sms, "containsLink") shouldBe true
    }

    @Test fun `containsFinancialKeyword`() {
        SmsEventFields.lookup(sms, "containsFinancialKeyword") shouldBe false
    }

    @Test fun `containsSecurityKeyword`() {
        SmsEventFields.lookup(sms, "containsSecurityKeyword") shouldBe true
    }

    @Test fun `smsCategory as String`() {
        SmsEventFields.lookup(sms, "smsCategory") shouldBe "OTP"
    }

    @Test fun `unknown field returns null`() {
        SmsEventFields.lookup(sms, "ghost") shouldBe null
    }
}

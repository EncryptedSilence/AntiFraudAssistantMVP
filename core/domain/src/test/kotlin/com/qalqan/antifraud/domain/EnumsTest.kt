...
class EnumsTest {
    @Test fun `trust statuses include the spec set`() {
        TrustStatus.entries.map { it.name }.toSet() shouldBe
            setOf(
    ...
    @Test fun `domain statuses include the spec set`() {
        DomainStatus.entries.map { it.name }.toSet() shouldBe
            setOf(
    ...
    @Test fun `call directions include the spec set`() {
        CallDirection.entries.map { it.name }.toSet() shouldBe
            setOf(
    ...
    @Test fun `session statuses match spec section 3 dot 2`() {
        SessionStatus.entries.map { it.name }.toSet() shouldBe
            setOf(
    ...
    @Test fun `campaign statuses match spec section 16 dot 7`() {
        CampaignStatus.entries.map { it.name }.toSet() shouldBe
            setOf(

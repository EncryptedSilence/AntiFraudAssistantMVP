...
data class DemoFixture(
    ...
    companion object {
        ...
        fun fromJson(json: String): DemoFixture =
            requireNotNull(adapter.fromJson(json)) { "could not parse demo fixture" }
    }
}
...
private class DemoEventAdapter {
    ...
    fun fromJson(reader: JsonReader): DemoEvent {
        val raw = reader.readJsonValue() as? Map<*, *>
    ...
    fun toJson(writer: JsonWriter, event: DemoEvent) {
        writer.beginObject()
        when (event) {
            is DemoEvent.Call -> {
